...
1. **Offline verification** — проєкт можна запустити без API ключа.
...
Mock provider не є фінальною AI-логікою. Він потрібен як стабільний fallback для тестів, локального запуску і перевірки без зовнішніх секретів.
...
Ці речі можна додати в майбутньому, але для поточного MVP головний фокус зроблено на core AI Engineering workflow.
